...
SNAP_API_URL = 'https://adsapi.snapchat.com/v1'
...
class SnapchatAdsAdapter(BaseAdAdapter):
    ...
    def authenticate(self) -> str:
        ...
        resp = self.session.post(SNAP_TOKEN_URL, data={
    ...
    def fetch_campaign_data(self, date_from, date_to):
        ...
        campaigns_url = f"{SNAP_API_URL}/adaccounts/{ad_account_id}/campaigns"
        ...
        for c in camp_data.get('campaigns', []):
            sub = c.get('campaign', {})
            campaign_map[sub.get('id')] = sub.get('name', '')

        # Step 2: Get stats for each campaign
        rows = []
        for campaign_id, campaign_name in campaign_map.items():
            stats_url = f"{SNAP_API_URL}/campaigns/{campaign_id}/stats"
            params = {
                'granularity': 'DAY',
            ...
            try:
                ...
                for ts in stats_data.get('timeseries_stats', []):
                    for series in ts.get('timeseries_stat', {}).get('timeseries', []):
                        ...
                        rows.append({
                            ...
                            'clicks': int(stats.get('swipes', 0)),  # Snapchat = swipes
                            'spend': int(stats.get('spend', 0)) / 1_000_000,  # microcurrency
                            'conversions': int(stats.get('conversion_purchases', 0)),
                        })
            except Exception as e:
                _logger.warning("Snapchat: failed to get stats for campaign %s: %s", campaign_id, e)
